...
Default_Pram={
        ...
        'Pram_initLen' : 100,
        # 'Pram_trainLen' : 200,
        # 'Pram_testLen':6000,#used to determine simulation end times
        'Pram_trainLen' : 200,
        'Pram_testLen':6000,#used to determine simulation end times
...
class EchoStateNetwork:
    ...
    def __init__(self,Pram,Oram):
        ...
        dataX,dataY=self.loadData(name,offset=0)
        U_in,Y_out=self.H_EncodeData(dataX,dataY, self.testLen)
        self.U_in=U_in
        self.Y_out=Y_out
        ...
        return
    def UpdateTrainData(self,offset):
        self.trainLen=self.trainLen+offset
        self.U_train=self.U_in[:, self.initLen: self.trainLen]
        self.Y_train=self.Y_out[:, self.initLen: self.trainLen]
        self.TrainProcessLen= self.trainLen- self.initLen
    ...
    # @numba.jit
    def Validate_test_data_Gap(self, U_test,gap):     
        # Length of U_test is Gap 
        alaph=self.galaph
        X_test=zeros((sum(self.X_dim),gap))
        for i in range(gap):
...
class ESNsimulater:
    ...
    def Increasingly_evaluateMergeHE(self,ldir='./default/',method=1,Q2=0):
        ...
        # T_Simulation_MSE[0,T_Simulation_Timer]=mse[1]
        #span
        ESN_0.Init_reservior(ESN_0.U_init)
        traintime=time.time()
        TrainProcessX=ESN_0.Train_reservoir(ESN_0.U_train,ESN_0.Y_train)
        ...
        # testmse=ESN_2.Verify_Train_reservoir(X_test,Y_p,False)
        #Record 
        T_Simulation_Predict_Test[:,T_Simulation_Timer:T_Simulation_Timer+Gap]=Y_out
        ...
        savetxt(ldir+'finalSimuTest.txt',T_Simulation_Predict_Train.T, fmt='%f', delimiter=',')  
        savetxt(ldir+'finalTrain.txt',ESN_0.Y_out.T, fmt='%f', delimiter=',')    
        savetxt(ldir+'finalMse.txt',T_Simulation_MSE.T, fmt='%f', delimiter=',')     
    ...
    @numba.jit
    def T_Simulationreasingly_Jump_Repeat(self,cdir='./cache',redotime=1):
...
if __name__ == '__main__':
    ...
    #PSO wi
        Proceduledir=Datesetdir+'PSO/'+'ampwi/'
        Es=ESNsimulater(Pram=Default_Pram,Oram=Default_Oram) 
        Es.mkdir(Proceduledir)
    ...
    if(0 and Pre_def_PSO):
        ...
        Proceduledir=Datesetdir+'PSO/'+'ampwo/'
        Es=ESNsimulater(Pram=Default_Pram,Oram=Default_Oram) 
        Es.mkdir(Proceduledir)
    ...
    if(1):    
        ...
        for i in range(REP):
            # Es.Increasingly_evaluateMergeHE(Proceduledir+'m2/'+str(i)+'/',method=2,Q2=0)
            Es.Increasingly_evaluateMergeHE(Proceduledir+'m3/'+str(i)+'/',method=3,Q2=0)
