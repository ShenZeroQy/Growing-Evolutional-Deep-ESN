...
# predefed Pram
Default_Pram={
        'Exp_dada_file':'./data/MackeySunmonth.txt',
...
def GUI_sig_and_err(self,Yo,Ys,TrainLen,dir,id:int):
        dim,ShowLen=shape(Yo)
        fig, axis = plt.subplots(2, sharex=True,gridspec_kw={'height_ratios': [2,1]})    
        fs=12
        fig.set_figheight(5)
        fig.set_figwidth(8)
        ...
        for j in range(dim):
            fig.clear()
            axis[0].plot(range(0,ShowLen), Ys[j:j+1,0:ShowLen].T, ls="-", lw=1,color='b', label='Target')
            axis[0].plot(range(TrainLen), Yo[j:j+1,0:TrainLen].T, ls="dashed", lw=1,color='y',label='Train')
            axis[0].plot(range(TrainLen,ShowLen), Yo[j:j+1,TrainLen:ShowLen].T, ls="dashed", lw=1,color='r', label='Test') 
            axis[1].plot(range(ShowLen), (Ys[j:j+1,0:ShowLen]-Yo[j:j+1,0:ShowLen]).T, ls="-", lw=1,color='g', label='Error') 
        axis.legend(loc=0)
        # axis[0].set_ylabel(r'A axis/deg',fontsize=fs)
        # axis[1].set_ylabel(r'E axis/deg',fontsize=fs)
        axis[1].set_xlabel('Time step',fontsize=fs)
        
        
        lines, labels = axis[0].get_legend_handles_labels()
        aline, alables = axis[1].get_legend_handles_labels()
        lines.extend(aline)
        ...
        fig.show()
        fig.set_rasterized(True)
        plt.savefig(dir+'prediction_and_err'+str(id)+'.eps') 
...
if __name__ == '__main__':
    Datesetdir='./SSM/16P10P15/'
    
    if(0 and Pre_def_PSO):
    #PSO wi
        Proceduledir=Datesetdir+'psowi/'
        pso = PSO(func=pso_func_wi, dim=1, pop=20, max_iter=10, lb=[0.00001], ub=[1], w=0.8, c1=0.5, c2=0.5)
        pso.run()
        opx=pso.gbest_x
        opres=pso.gbest_y
        print(opx)
    ...
    if(0 and Pre_def_PSO):
        ...
        pso = PSO(func=pso_func_wp, dim=1, pop=10, max_iter=10, lb=[0.5], ub=[5], w=0.8, c1=0.5, c2=0.5)
        pso.run()
        opx=pso.gbest_x
        opres=pso.gbest_y
        print(opx)
