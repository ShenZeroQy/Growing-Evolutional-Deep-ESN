...
def denemicmerge(ldir):
    #evadir or rep dir
    MSEtest=zeros((REP,2))
    MSEtrain=zeros((REP,2))
    for i in range(REP):
        fs=12
        cdir=ldir+str(i)+'/'
        initlen=200
        trainlen=2000
        testlen=6000
        dxd=loadtxt(cdir+'finalSimuTrain.txt',dtype=double, delimiter=',')
        dyd=loadtxt(cdir+'finalSimuTest.txt',dtype=double, delimiter=',')
        dtd=loadtxt(cdir+'finalTrain.txt',dtype=double, delimiter=',')
        #dynamic trainlen
        Nonzero=argwhere(dxd!=0)
        last=Nonzero[-1]
        trainlen=last[0]+1
        initlen=Nonzero[0][0]
        fig,(axia1,axib1,axia2,axib2)=plt.subplots(4,sharex=True,gridspec_kw={'height_ratios': [3,1.2,3,1.2]})
        fig.subplots_adjust(hspace=0.2)
        fig.set_figheight(5)
        fig.set_figwidth(8)
        xtrain=[k/100 for k in range(initlen,trainlen)]
        xtest=[k/100 for k in range(trainlen,testlen)]
        dy=dxd[initlen:trainlen,0]
        dt=dtd[initlen:trainlen,0]
        MSEA=(dy-dt)**2
        MSEA=sum(MSEA)/len(dy)
        axia1.plot(xtrain,dt,label='Target',color='b')
        axia1.plot(xtrain,dy,label='Prediction',color='r',linestyle='dashed')
        axib1.plot(xtrain,dy-dt,label='Error',color='g',linestyle='-')
        
        dy=dxd[initlen:trainlen,1]
        dt=dtd[initlen:trainlen,1]
        MSEB=(dy-dt)**2
        MSEB=sum(MSEB)/len(dy)
        
        MSEtrain[i,0]=MSEA
        MSEtrain[i,1]=MSEB
        
        axia2.plot(xtrain,dt,label='Target',color='b')
        axia2.plot(xtrain,dy,label='Prediction',color='r',linestyle='dashed')
        axib2.plot(xtrain,dy-dt,label='Error',color='g',linestyle='-')
        # axia1.legend(fontsize=fs,ncol=2,frameon=False)
        # axib1.legend(fontsize=fs,ncol=1,frameon=False)
        axia1.set_ylabel(r'A axis/deg',fontsize=fs)
        
        axia2.set_ylabel(r'E axis/deg',fontsize=fs)
        axib2.set_xlabel('t/s',fontsize=fs)
        # axia2.legend(fontsize=fs,ncol=2,frameon=False)
        # axib2.legend(fontsize=fs,ncol=1,frameon=False)

        # plt.tight_layout() 
        lines, labels = axia1.get_legend_handles_labels()
        aline, alables = axib1.get_legend_handles_labels()
        lines.extend(aline)
        labels.extend(alables)
        fig.legend( lines, labels,           
            bbox_to_anchor=(0.74, 0.96),ncol=3, framealpha=1)
        fig.show()
        plt.savefig(cdir+'Trainingerr.eps')
        plt.close(fig)
        fig.clear()
        fig,(axia1,axib1,axia2,axib2)=plt.subplots(4,sharex=True,gridspec_kw={'height_ratios': [3,1.2,3,1.2]})
        ...
        fig.set_figheight(5)
        fig.set_figwidth(10)
        dy=dyd[trainlen:testlen,0]
        dt=dtd[trainlen:testlen,0]
        MSEA=(dy-dt)**2
        MSEA=sum(MSEA)/len(dy)
        axia1.plot(xtest,dt,label='Target',color='b')
        axia1.plot(xtest,dy,label='Prediction',color='r',linestyle='dashed')
        axib1.plot(xtest,dy-dt,label='Error',color='g',linestyle='-')
        dy=dyd[trainlen:testlen,1]
        dt=dtd[trainlen:testlen,1]
        MSEB=(dy-dt)**2
        MSEB=sum(MSEB)/len(dy)
        ...
        axia2.plot(xtest,dt,label='Target',color='b')
        axia2.plot(xtest,dy,label='Prediction',color='r',linestyle='dashed')
        axib2.plot(xtest,dy-dt,label='Error',color='g',linestyle='-')
        # axia3.legend(fontsize=fs)
        
        axia1.set_ylabel(r'A axis/deg',fontsize=fs)
        axia2.set_ylabel(r'E axis/deg',fontsize=fs)
        axib2.set_xlabel('t/s',fontsize=fs)
        # axia2.legend(fontsize=fs)
        ...
        fig.legend( lines, labels,           
            bbox_to_anchor=(0.74, 0.96),ncol=3, framealpha=1)
        fig.show()
        plt.savefig(cdir+'Testingerr.eps')
        plt.close(fig)
        
    averMSEtest=mean(MSEtest,axis=0)   
    averMSEtrain=mean(MSEtrain,axis=0)  
    print(averMSEtrain) 
    print(averMSEtest) 
    savetxt(ldir+'MSEtest.txt',MSEtest,fmt='%1.9f',delimiter='\t')
    savetxt(ldir+'MSEtrain.txt',MSEtrain,fmt='%1.9f',delimiter='\t')
